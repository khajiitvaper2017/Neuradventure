  ...
  async function generate() {
    ...
    try {
      const result = await api.generate.story(
        $pendingStoryGenerateDescription.trim(),
  ...
  <div class="form-scroll" data-scroll-root="screen">
    ...
    <div class="field">
      ...
      <div class="saved-row">
          ...
          {#if showCharacterDropdown}
            <div class="saved-select-menu" role="listbox">
              {#each savedCharacters as c}
                <button class="saved-select-item" role="option" onclick={() => selectCharacter(c.key)}>
                  {c.character.name}
                </button>
              {/each}
            </div>
        ...
        <button class="btn-ghost" onclick={useSavedCharacter} disabled={!selectedCharacterKey}>
          Use
        </button>
        <button class="btn-ghost" onclick={() => activeScreen.set("char-create")}>
          New
        </button>
        <button class="btn-ghost" onclick={loadCharacters} disabled={loadingCharacters}>
          Refresh
        </button>
      </div>
    </div>
    ...
    {:else}
      <div class="char-summary empty">
        <div class="char-summary-header">Character</div>
        <div class="char-details">No character selected yet.</div>
        <button class="btn-accent small" onclick={() => activeScreen.set("char-create")}>
          New Character
        </button>
      </div>
    {/if}
