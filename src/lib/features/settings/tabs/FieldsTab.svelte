<script lang="ts">
  ...
  async function loadAll() {
    if (loadingDefs || loadingOverrides) return
    error = null
    loadingDefs = true
    loadingOverrides = true
    try {
      const [d, o] = await Promise.all([settingsService.customFields(), settingsService.fieldPromptOverrides()])
      defs = d
      overrides = o
      loadedOnce = true
    } catch (err) {
      console.error("[fields] Failed to load", err)
      error = err instanceof Error ? err.message : "Failed to load field settings."
    } finally {
      loadingDefs = false
  ...
  const builtinKeys = Object.keys(promptFields as Record<string, string>).sort((a, b) => a.localeCompare(b))
  let promptSearch = $state("")
  let selectedKey = $state<string>(builtinKeys[0] ?? "")
  ...
  const filteredKeys = $derived.by(() => {
    ...
    if (!q) return builtinKeys
    return builtinKeys.filter((k) => k.toLowerCase().includes(q))
  })

  $effect(() => {
    if (!selectedKey) return
    const current = overrides[selectedKey] ?? ""
    overrideDraft = current
    overrideDirty = false
  })
  ...
  async function saveOverride() {
    ...
    error = null
    try {
      overrides = await settingsService.setFieldPromptOverride(selectedKey, overrideDraft)
      overrideDirty = false
    } catch (err) {
      console.error("[fields] Failed to save override", err)
  ...
  async function resetOverride() {
    ...
    error = null
    try {
      overrides = await settingsService.resetFieldPromptOverride(selectedKey)
      overrideDirty = false
      overrideDraft = overrides[selectedKey] ?? ""
    } catch (err) {
      console.error("[fields] Failed to reset override", err)
  ...
  async function resetAllOverrides() {
    ...
    try {
      ...
      console.error("[fields] Failed to reset all overrides", err)
...
<div class="space-y-4">
  ...
  <Card>
    <CardHeader>
      <CardTitle>Field Prompt Overrides</CardTitle>
      <CardDescription
        >Override built-in JSON schema descriptions from <span class="font-mono">prompt-fields.json</span
        >.</CardDescription
      >
    </CardHeader>
    <CardContent class="space-y-4">
      <div class="grid gap-3 md:grid-cols-[1fr_1.4fr]">
        <div class="space-y-2">
          <Label for="prompt-search">Search keys</Label>
          <Input
            id="prompt-search"
        ...
        <div class="space-y-3">
          <div class="space-y-2">
            ...
            <Textarea rows={6} value={(promptFields as Record<string, string>)[selectedKey] ?? ""} disabled />
          </div>
          <div class="space-y-2">
            ...
            <Textarea
              ...
              disabled={!selectedKey || overrideSaving || loadingOverrides}
            />
          </div>
