...
{#snippet renderTokens(text)}
  {#each tokenizeInline(text) as token, tokenIndex (tokenIndex)}
    {#if token.type === "text"}
      {token.content}
    ...
    {:else if token.type === "quote"}
      <span class="text-dialogue">"""{@render renderTokens(token.content)}"""</span>
