...
import { MainCharacterStateStoredSchema, NPCStateStoredSchema, WorldStateStoredSchema } from "../engine/core/models.js"
import { createNewStory } from "../engine/game/index.js"
import {
  TavernCardV2Schema,
...
import { normalizeStoryModules, StoryModulesSchema } from "../engine/schemas/story-modules.js"
...
export const stories = {
  ...
  create: async (data: {
    ...
    if (data.character_id) {
      ...
      if (!charRow) throw new ApiError(404, "Character not found")
      ...
      if (data.tavern_card !== undefined && data.tavern_card !== null) {
        ...
        if (!parsedCard.success) throw new ApiError(400, "Invalid tavern_card payload")
      ...
      throw new ApiError(400, "Provide character_id or character_data")
    }
  ...
  updateState: async (
    ...
    const nextWorld = data.world
      ? { ...currentWorld, ...(data.world.memory !== undefined ? { memory: data.world.memory } : {}) }
      : currentWorld
  ...
  import: async (data: object | string): Promise<{ id: number }> => {
    ...
    if (format === "tavern-card") {
      throw new ApiError(400, "Character card detected. Use character import instead.")
    }
    ...
    throw new ApiError(400, "Unrecognized import format. Expected Neuradventure JSON or SillyTavern JSONL.")
  },
  ...
  getCharacter: async (id: number): Promise<MainCharacterState> => {
    if (!Number.isFinite(id) || id <= 0) throw new ApiError(400, "Invalid character id")
    const charRow = db.getCharacter(id)
    if (!charRow) throw new ApiError(404, "Character not found")
    try {
      return MainCharacterStateStoredSchema.parse(JSON.parse(charRow.state_json))
    } catch {
      throw new ApiError(400, "Stored character state is invalid JSON")
    }
  },
  ...
  getCharacterCard: async (charId: number): Promise<object> => {
    ...
    if (!row) throw new ApiError(404, "No stored character card")
    try {
      ...
      throw new ApiError(400, "Stored character card is invalid JSON")
    }
  },
  ...
  importCharacter: async (body: object): Promise<CharacterImportResult> => {
    ...
    if ("png_base64" in record || "png_data_url" in record) {
      ...
      if (!base64) throw new ApiError(400, "Invalid PNG payload. Expected png_base64 or png_data_url.")

      let extracted: { card: unknown }
      try {
        extracted = extractCardJsonFromPng(base64ToBytes(base64))
      } catch (err) {
        throw new ApiError(400, err instanceof Error ? err.message : "Invalid PNG character card")
      }

      const originalCard = extracted.card as unknown
      const parsedCard = TavernCardV2Schema.safeParse(originalCard)
      if (!parsedCard.success) throw new ApiError(400, "Invalid embedded character card JSON")
    ...
    throw new ApiError(400, "Unrecognized import format. Expected TavernCardV2 or Neuradventure character.")
  },
}
