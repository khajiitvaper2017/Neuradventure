...
import { getServerDefaults } from "../engine/core/strings.js"
import {
  buildChatMessages,
  buildChatStopTokens,
  sanitizeChatReply,
  type ChatMember,
  type ChatMessage,
} from "../engine/llm/chat.js"
import { generateChatReply } from "../engine/llm/index.js"
import { createOrGetSession, publishComplete, publishError, publishPreview } from "../engine/streaming/hub.js"
import {
  chatToPlaintext,
  chatToTavernJSONL,
  TavernCardV2Schema,
  type TavernCardV2,
} from "../engine/utils/converters/tavern.js"
import type {
  ChatCancelResult,
  ChatContinueResult,
...
import type { ChatDetail, ChatMessage as ChatMessagePayload, ChatSummary } from "../../../shared/types.js"
...
function getCachedOrInFlight<T>(requestId: string, kind: string): { cached?: T; inflight?: Promise<T> } {
  ...
  if (cached) {
    if (cached.kind !== kind) throw new ApiError(409, `request_id already used for: ${cached.kind}`)
...
export const chats = {
  ...
  updateMessage: async (chatId: number, messageId: number, content: string): Promise<ChatUpdateMessageResult> => {
    ...
    if (!message || message.chat_id !== chatId) throw new ApiError(404, "Message not found")
    ...
    if (!updated) throw new ApiError(500, "Failed to update message")
  ...
  create: async (data: {
    ...
    if (playerMembers.length !== 1) throw new ApiError(400, "Exactly one player member is required")
    if (aiMembers.length < 1) throw new ApiError(400, "At least one AI member is required")

    if (data.seed_greeting) {
      const index = data.seed_greeting.speaker_sort_order
      const seedTarget = members[index]
      if (!seedTarget) throw new ApiError(400, "seed_greeting.speaker_sort_order is out of range")
      if (seedTarget.role !== "ai") throw new ApiError(400, "seed_greeting target must be an AI member")
    }
  ...
  send: async (id: number, content: string, requestId?: string): Promise<ChatSendResult> => {
    ...
    if (!playerMember) throw new ApiError(400, "Chat has no player member")
    const aiMembers = listAiMembers(members)
    if (aiMembers.length === 0) throw new ApiError(400, "Chat has no AI members")

    const trimmedContent = content.trim()
    if (!trimmedContent) throw new ApiError(400, "Message content is required")
    ...
    try {
      ...
      throw new ApiError(500, "LLM generation failed: " + message)
    }
  },
  ...
  regenerateLast: async (id: number, requestId?: string): Promise<ChatRegenerateResult> => {
    ...
    if (allMessages.length === 0) throw new ApiError(400, "Chat has no messages")
    ...
    if (!speakerMember) throw new ApiError(500, "Failed to resolve speaker")
  ...
  cancelLast: async (id: number): Promise<ChatCancelResult> => {
    ...
    const aiMembers = listAiMembers(members)
    const messages = db.listChatMessages(id)
    if (messages.length === 0) throw new ApiError(400, "Chat has no messages")
  ...
  undoCancel: async (id: number): Promise<ChatUndoCancelResult> => {
    ...
    if (!payload) throw new ApiError(400, "Nothing to undo")
  ...
  setNextSpeaker: async (id: number, speakerMemberId: number): Promise<ChatSetNextSpeakerResult> => {
    ...
    if (idx < 0) throw new ApiError(404, "Speaker not found")
