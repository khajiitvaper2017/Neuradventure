...
import { getCachedUpstreamModels } from "../engine/llm/models.js"
...
const builtinPresetModules = import.meta.glob("../../../shared/config/presets/*.json", { eager: true }) as Record<
  string,
...
export const settings = {
  ...
  upsertPreset: async (preset: Omit<SamplerPreset, "id">): Promise<SamplerPreset> => {
    if (!preset.name.trim()) throw new ApiError(400, "Preset name is required")
  ...
  deletePreset: async (id: number): Promise<{ ok: boolean }> => {
    ...
    if (!ok) throw new ApiError(404, "Preset not found")
    return { ok: true }
  },
}
